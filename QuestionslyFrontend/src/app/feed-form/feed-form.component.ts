...
export class FeedFormComponent implements OnInit {
    ...
    defaultages: any[] = [];
    plotselection: any = {age: this.defaultages, location: [], gender: ['male', 'female', 'unknown']};

    // ages
    unknownage: any[] = ['Unknown'];
    valuesagesmin17: any[] = [];
    valuesages17to23: any[] = [];
    valuesages24to29: any[] = [];
    valuesages30to39: any[] = [];
    valuesages40to49: any[] = [];
    valuesages50to59: any[] = [];
    valuesplus60: any[] = [];
    ...
    postForm(data) {
        ...
        (window as any).mixpanel.track('Answered Question', {
    ...
    expand() {
        ...
        (window as any).mixpanel.track('Clicked "See More"', {
            'timeElapsedFromInit': (Date.now() - startingTime) / 1000,
            'question': this.form.questions[0].body,
            'id': this.form.id,
            'timestamp': Date.now()
...
// NEW: THIS IS THE FUNCTION THATS CALLED WHEN THE FORM CONTROL IS CHANGED
    doDataSelectionUpdate(x) {
        ...
        if (x.itemName !== 'Unknown' && x.input == 'age') {
            ...
            const tempgenderstring2 = [x.itemName.toLowerCase()];

            if (x.status == false) {
                this.plotselection.gender = this.plotselection.gender.filter(function(e) {return tempgenderstring2.indexOf(e) == -1});
            } else {
                this.plotselection.gender = this.plotselection.gender.concat(tempgenderstring2);
            }
